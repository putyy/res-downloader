...
使用 JSON 时，可以查看[Manifest Schema 和编辑器类型声明](plugin-sdk.md)获得字段提示；应用的 `plugin lint` 校验结果始终是最终标准。
...
fixture 可以用单个 `observation`，也可以用 `observations` 顺序回放页面接口、视频、音频和滚动加载等多请求会话；同一 `groupKey` 会按真实运行语义聚合。测试支持 `resourceCount`、`resourceUrls`、`processorTypes`、`decision` 和 `patchBodyContains`。模板位于 `examples/plugins/`，JSON Schema 和 TypeScript 声明见 [插件 SDK v1](plugin-sdk.md)。
