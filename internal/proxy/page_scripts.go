...
func buildPageScriptTag(script shared.PageScriptInjection, nonce string) string {
	...
	tag.WriteString(`commands:Object.freeze({claim:function(requestId,resumeToken){return commandRequest("claim",{requestId:requestId,resumeToken:resumeToken});},report:function(requestId,report){return commandRequest("report",{requestId:requestId,state:report.state,progress:report.progress,message:report.message});}}),`)
	...
	tag.WriteString(`function commandRequest(action,data){if(!bridgeBase||!nativeFetch)return Promise.reject(new Error("page bridge is unavailable"));var body=JSON.stringify(data);if(body.length>4096)return Promise.reject(new Error("page command report is too large"));return nativeFetch(bridgeBase+"command-"+action,{method:"POST",headers:{"Content-Type":"application/json"},body:body,credentials:"same-origin",cache:"no-store"}).then(function(response){return response.json().then(function(result){if(!response.ok||!result.ok)throw new Error(result.error||"page command failed");return result;});});}`)
